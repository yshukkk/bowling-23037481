...
목표를 정하고 Plan.md로 사람 파트너와 합의하라. 테스트를 먼저 작성하라. 실패하는 것을 확인하라. 통과시킬 최소한의 코드를 작성하라. Plan 대비 초과 구현이 없는지 리뷰하라.
...
human-in-the-loop을 강조하는 Agentic Engineering TDD 사이클. 실패하는 테스트 이전에 목표와 계획을 사람 파트너와 먼저 맞추고, 구현 이후에는 계획 대비 초과 구현이 없는지 리뷰한다.

```dot
digraph tdd_cycle {
    rankdir=LR;
    plan [label="RED-1\n목표 설정 +\nPlan.md 생성", shape=box, style=filled, fillcolor="#ffe0cc"];
    verify_plan [label="Plan.md\n사람 검토", shape=diamond];
    commit_plan [label="Plan.md\n커밋", shape=box, style=filled, fillcolor="#ffe0cc"];
    red [label="RED-2\n실패하는 테스트 작성", shape=box, style=filled, fillcolor="#ffcccc"];
    verify_red [label="제대로\n실패하는지 확인", shape=diamond];
    green [label="GREEN\nPlan의 goal을\n달성하는 최소 코드", shape=box, style=filled, fillcolor="#ccffcc"];
    verify_green [label="통과 확인\n전체 그린", shape=diamond];
    review [label="REVIEW\nPlan 대비 코드 검토", shape=box, style=filled, fillcolor="#ffffcc"];
    commit_code [label="코드\n커밋", shape=box, style=filled, fillcolor="#ffffcc"];
    refactor [label="REFACTOR\n정리", shape=box, style=filled, fillcolor="#ccccff"];
    next [label="다음", shape=ellipse];

    plan -> verify_plan;
    verify_plan -> commit_plan [label="승인"];
    verify_plan -> plan [label="수정 요청"];
    commit_plan -> red;
    red -> verify_red;
    ...
    review -> refactor [label="초과 구현/\n리팩토링 필요"];
    review -> commit_code [label="Plan과 일치"];
...
### RED-1 - 목표 설정 및 Plan.md 생성/검토

테스트를 쓰기 전에, 이번 증분에서 무엇을 달성할지부터 명확히 한다.

1. 이번 증분의 목표(goal)를 정확히 설정한다 — 다음에 추가할 가장 작은 동작 하나 (예: gutter game, 스페어 하나, 스트라이크 하나 등).
2. `Plan.md`(또는 해당 증분의 계획 문서)를 생성해 다음을 적는다:
   - 목표(goal)
   - 추가/변경할 테스트 케이스 개요
   - 구현 범위 (그리고 명시적으로 범위 밖인 것)
3. 사람 파트너에게 `Plan.md` 검토를 요청하고, 승인 없이 다음 단계로 넘어가지 않는다.
4. 승인되면 `Plan.md`를 커밋한다 (아래 "## 커밋 시점" 참고).

**주의:** Plan.md 검토를 건너뛰고 바로 테스트를 작성하는 것은 human-in-the-loop 원칙 위반이다. 사소해 보이는 증분이라도 생략하지 않는다.

### RED-2 - 실패하는 테스트 작성

Plan.md가 승인된 뒤, 무엇이 일어나야 하는지를 보여주는 최소한의 테스트 한 개를 작성하라.
...
**Plan.md의 goal만 달성한다.** RED-1에서 승인받은 범위를 벗어나는 구현(추가 메서드, 추가 검증, 다음 증분 선반영 등)은 하지 않는다. 범위를 벗어나고 싶다면 REVIEW 단계에서 사람 파트너에게 먼저 알린다.
...
### REVIEW - Plan 대비 코드 검토

**필수. GREEN 검증 직후, REFACTOR 이전에 수행한다.**

GREEN에서 나온 코드를 Plan.md와 대조하며 리뷰한다:
- Plan.md에 없는 사항이 구현되지는 않았는가? (초과 구현)
- 초과 구현이 있다면: 제거하거나, 왜 필요했는지 사람 파트너에게 설명하고 별도 승인을 받는다.
- 리팩토링이 필요해 보이는가? 필요하다면 사람 파트너에게 리팩토링을 요청한다 (바로 수행하지 않고 먼저 요청한다).

리뷰가 끝나고 Plan과 일치함이 확인되면, 코드를 커밋한다 (아래 "## 커밋 시점" 참고).

### REFACTOR - 정리

REVIEW에서 리팩토링이 필요하다고 판단된 경우에만, 그린 상태를 유지하며:
- 중복 제거
- 이름 개선
- 헬퍼(helper) 추출
...
다음 기능에 대한 목표 설정과 `Plan.md` 작성(RED-1)부터 다시 시작하라.

## 커밋 시점

Agentic Engineering TDD 수행을 추적할 수 있도록, 각 단계 완료 시점에 커밋을 남긴다.

| 단계 | 커밋 대상 | 시점 |
|------|-----------|------|
| RED-1 | `Plan.md` | 사람 파트너가 Plan.md를 승인한 직후 |
| REVIEW | 구현 코드 (+ 테스트) | Plan 대비 검토를 마치고 초과 구현이 없음을 확인한 직후 |

- 커밋 메시지에 어느 단계의 산출물인지 드러낸다 (예: `RED: plan for spare scoring`, `GREEN+REVIEW: implement spare scoring`).
- REFACTOR로 코드가 바뀐 경우, 별도 커밋으로 남기거나 REVIEW 커밋에 포함할지 사람 파트너와 합의한다.
...
- "TDD는 교조적, 나는 실용적"
- "이건 다른 경우인데..."
- Plan.md 검토 없이 바로 테스트/코드를 작성했다
- Plan.md 범위를 벗어난 구현을 REVIEW 없이 그냥 커밋했다
...
## 목표
`submit_form`이 빈 이메일(공백 포함)을 받으면 에러를 반환한다.

## 추가할 테스트
- test_rejects_empty_email: email=""

## 범위
- submit_form의 이메일 공백 검증만. 다른 필드 검증은 범위 밖.
```
→ 사람 파트너 검토 및 승인 → 커밋
...
**REVIEW**
구현이 Plan.md의 범위(이메일 공백 검증만)를 벗어나지 않았음을 확인 → 커밋
...
- [ ] `Plan.md`가 RED-1 단계에서 생성되고 사람 파트너의 검토·승인을 거쳤다
- [ ] 승인된 `Plan.md`가 커밋되었다
...
- [ ] GREEN 코드가 `Plan.md` 범위를 벗어나지 않았다 (벗어났다면 REVIEW에서 별도로 식별·승인됨)
- [ ] REVIEW를 마친 코드가 커밋되었다
...
```
프로덕션 코드 → Plan.md가 승인되었고, 테스트가 존재하고, 먼저 실패했고, REVIEW를 거쳤다
그 외 → TDD가 아니다
```
